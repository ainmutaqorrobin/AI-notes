...
http.route({
  ...
  handler: httpAction(async (ctx, req) => {
    ...
    const result = streamText({
      ...
      You have to provide links (if you found) to relevant notes using this relative URL structure (omit the base URL): '/notes?noteId=<note-id>'.
      Keep your responses concise and to the point.`,
      messages: convertToModelMessages(lastMessages),
      tools: {
        findRelatedNotes: tool({
          description:
          ...
          execute: async ({ query }) => {
            console.log("findRelatedNotes query:", query);
            const relatedNotes = await ctx.runAction(
              internal.notesActions.findRelatedNotes,
              {
                query,
                userId,
              }
